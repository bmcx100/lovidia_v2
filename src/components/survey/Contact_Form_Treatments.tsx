...
import { TreatmentsFormSchema } from "@/constants/contactData";
...
const Contact_Form_Treatments = ({ currentPage, totalPages, handleNext, handlePrev }: ContactFormProps) => {
  const form = useForm<z.infer<typeof TreatmentsFormSchema>>({
    resolver: zodResolver(TreatmentsFormSchema),
    defaultValues: {
      treatments: "",
      management: "",
      treatmentComments: "",
  ...
  const thisHandleSubmit = (values: z.infer<typeof TreatmentsFormSchema>) => {
    console.log({ values });
  };
    ...
    <Card className=" ">
      <CardHeader className="m-0 overflow-clip bg-brandSecondary  px-4 py-4">
        <div className="flex flex-row items-start justify-between">
          <CardTitle className="m-0 text-xl font-medium">Treatments</CardTitle>
        </div>
          ...
          >
            <FormField
              ...
              render={({ field }) => {
                return (
                  <FormItem>
                    <FormLabel className="capitalize">Impact of medications and therapies:</FormLabel>
                    <FormControl>
                      <Textarea
                        className=""
                        {...field}
            ...
            <FormField
              ...
              render={({ field }) => {
                return (
                  <FormItem>
                    <FormLabel className="capitalize">What has been helpful for you?</FormLabel>
                    <FormControl>
                      <Textarea
                        {...field}
                        placeholder="What actions or lifestyle changes have you found helpful in
                      managing your Long COVID symptoms?"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
